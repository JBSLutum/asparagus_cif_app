...
# Provide this function when you have funding data entered in the excel sheet in data folder
...
ui <- fluidPage(
  ...
  tags$div(
    ...
    tags$h2(tags$div("Klimafolgenprognosen:"),
            tags$div("Anbau von Nutzpflanzen im Jahr 2075"),
  ...
  sidebarLayout(
    sidebarPanel(width = 4,
                 ...
                 accordion(
                   ...
                   # saveLoadUI("savemod"),
                   accordion_panel(
                     title = "Projekt speichern/laden", icon = icon("folder-open"),
                     tagList(
                   ...
                   ### Crop filter ----
                   accordion_panel(
                     title = "Auswahl der Kulturpflanzen",
                   ...
                   ### Expertise filter ----
                   accordion_panel(
                     title = "Fachwissen auswählen ",
                     icon = icon("clipboard-question"),
                     tagList(
                       tags$h5(
                         "Fachwissen auswählen",
                         tags$span(
                           icon("circle-question"),
                           title = "Select your main expertise to view and edit only relevant variables.\nNot selecting any box shows all variables.\nDefaults apply to unselected categories in simulations.",
                           style = "cursor: help; margin-left: 8px;"
...
server <- function(input, output, session) {
  ...
  output$dynamic_element_ui <- renderUI({
    ...
    panels <- lapply(seq_along(data_list), function(j) {
      ...
      cats <- unique(c( cats_crop[cats_crop!=""], cats_exp[cats_exp!=""]))
